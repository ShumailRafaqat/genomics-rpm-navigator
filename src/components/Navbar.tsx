      ...
      <div className="nav-gradient px-4 py-3 md:py-0">
        <div className="container mx-auto flex items-center justify-between h-14">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-heading font-bold text-primary-foreground tracking-wide">Resources</span>
          </div>
            ...
            <div className="flex flex-col gap-2">
              {allOptions.map((name) => {
                ...
                return (
                  <button
                    key={name}
                    onClick={() => { onSelectCampaign(name); setIsMobileMenuOpen(false); }}
                    className={`w-full text-left px-5 py-3 rounded-2xl font-medium text-base transition-all flex items-center gap-3
                      ${isActive ? "bg-white/25 text-white" : "text-primary-foreground/80 hover:bg-white/10 hover:text-white"}`}
                  >
                    {name}
